...
-- Используем 24-битное простое число, чтобы избежать переполнения чисел с плавающей точкой в Lua.
local P = 16777213 -- Простое число
local G = 2        -- Генератор

-- Функция модульного возведения в степень (необходима для криптографии)
...
-- Генерация финального ключа сессии (Общий Секрет)
-- Вызывается с использованием чужого публичного ключа и своего приватного
